  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-takal-ink">My Deliveries</h1>
      ...
      {!loading && parcels.length > 0 && (
        <div className="grid grid-cols-2 gap-4">
          <div className="bg-white border border-takal-line rounded-xl p-4">
            <p className="text-xs text-takal-ink-soft">Parcels with you</p>
            <p className="text-3xl font-bold text-takal-ink mt-1">{parcels.length}</p>
          </div>
          <div className="bg-white border border-takal-line rounded-xl p-4">
            <p className="text-xs text-takal-ink-soft">Cash to collect in total</p>
            <p className="text-3xl font-bold text-takal-ink mt-1">
              {rs(
                parcels.reduce(
                  (sum, p) =>
      ...
      {loading ? (
        ...
        <div className="bg-white border border-takal-line rounded-xl p-10 text-center">
          <div className="w-14 h-14 mx-auto mb-4 rounded-full bg-slate-100 flex items-center justify-center">
            <Truck className="w-7 h-7 text-takal-disabled-text" />
          </div>
          <h2 className="text-lg font-bold text-takal-ink mb-1">
        ...
        <div className="space-y-3">
          {parcels.map((p) => (
            <div
              key={p.id}
              className="bg-white border border-takal-line rounded-xl p-5 flex flex-col sm:flex-row sm:items-start gap-4"
            >
              <div className="flex-1 min-w-0">
                ...
                <h3 className="text-lg font-bold text-takal-ink mt-0.5">{doorName(p)}</h3>
                ...
                {(p.receiver_phone || p.customer_phone) && (
                  <a
                    ...
                    className="inline-flex items-center gap-1.5 text-sm font-semibold text-takal-ink mt-1 hover:underline"
                  >
                ...
                <div className="mt-2.5">
                  {mapsLink(p) ? (
                    <a
                      ...
                      className="group inline-flex items-start gap-2 text-sm text-takal-ink hover:text-takal-ink"
                    >
                      ...
                      <span className="leading-relaxed underline decoration-slate-300 group-hover:decoration-slate-900">
                        ...
                        {p.address_details && (
                          <span className="block text-takal-ink-soft no-underline">
                            {p.address_details}
                          </span>
                        )}
                        <span className="block text-xs text-takal-disabled-text no-underline mt-0.5">
                          Tap to open in Google Maps
                        </span>
                      </span>
                    </a>
                  ) : (
                    <p className="text-sm text-takal-ink-soft leading-relaxed">
                ...
                {p.notes && p.notes.trim() && (
                  <div className="mt-2.5 bg-takal-page border border-takal-line rounded-lg px-3 py-2">
                    <p className="text-xs font-bold text-takal-ink-soft mb-0.5">
                      NOTE FROM THE CUSTOMER
                    </p>
                    <p className="text-sm text-takal-ink leading-relaxed">{p.notes}</p>
                  </div>
                )}
              ...
              <button
                onClick={() => openFor(p)}
                className="w-full sm:w-auto px-6 py-3.5 bg-takal-yellow text-takal-ink font-bold rounded-lg hover:bg-takal-yellow-dark transition whitespace-nowrap"
              >
                Mark delivered
              </button>
            </div>
      ...
      {open && (
        <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-md p-6">
            ...
            <p className="text-sm text-takal-ink-soft mb-5">
            ...
            <button
              onClick={confirm}
              disabled={busy}
              className="w-full mt-5 py-3.5 bg-takal-yellow text-takal-ink font-bold rounded-lg hover:bg-takal-yellow-dark transition disabled:opacity-60 inline-flex items-center justify-center gap-2"
            >
            ...
            <p className="text-xs text-takal-disabled-text mt-4 text-center leading-relaxed">
