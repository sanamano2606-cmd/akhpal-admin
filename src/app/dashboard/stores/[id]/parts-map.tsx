// ─────────────────────────────────────────────────────────────────────────────
// Where the shop is: the map, the pin, and saving the position.
//
  ...
  return (
    <div className="bg-white rounded-lg border border-takal-line p-5 space-y-4">
      <div className="flex items-start justify-between gap-3">
        <div>
          <h3 className="font-semibold text-takal-ink">Shop location on the map</h3>
      ...
      <div className="rounded-lg overflow-hidden border border-takal-line">
        <div ref={mapDiv} className="w-full h-80 bg-slate-100" />
        <div className="px-3 py-2 text-xs bg-takal-page border-t border-takal-line flex items-center justify-between gap-3">
      ...
      {pin && (
        <div className="rounded-lg border border-takal-line bg-takal-page p-3">
          <p className="text-xs font-medium text-takal-ink-soft mb-1">Address at this pin</p>
          {lookingUp ? (
            <p className="text-sm text-takal-ink-soft">Looking it up...</p>
          ) : lookedUp ? (
            <>
              <p className="text-sm text-takal-ink">{lookedUp}</p>
              <label className="flex items-start gap-2 mt-2 text-sm text-takal-ink">
                <input
                  type="checkbox"
                  checked={useLookedUp}
                ...
                <span>
                  ...
                  {String(store?.address || "").trim() && (
                    <span className="text-takal-ink-soft"> — replaces the address above</span>
                  )}
                </span>
              </label>
            </>
      ...
      <details className="text-sm">
        <summary className="cursor-pointer text-takal-ink-soft hover:text-takal-ink">
          Or enter the coordinates by hand
        </summary>
        <div className="mt-3 space-y-3">
          <div>
            <label className="block text-xs font-medium text-takal-ink-soft mb-1">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-medium text-takal-ink-soft mb-1">Latitude</label>
              <input
            ...
            <div>
              <label className="block text-xs font-medium text-takal-ink-soft mb-1">Longitude</label>
              <input
                type="text" inputMode="decimal"
